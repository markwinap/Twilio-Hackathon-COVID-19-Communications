...
exports.handler = async (event) => {
  ...
  const emailList = getEmailList(body.familyMembers);
  const phoneList = getMobileList(body.familyMembers);
  if (emailList.length > 0) {
    const emailParams = {
      personalizations: [
        {
          to: emailList,
          subject: `Message from patient ${body.selectedPatient.firstName} ${body.selectedPatient.lastName}`,
        },
      ],
      content: [
        {
          type: 'text/html',
          value: emailBody(
            `Message from patient ${body.selectedPatient.firstName} ${body.selectedPatient.lastName}`,
            body.note
          ),
        },
      ],
      from: {
        email: 'covid19@example.com',
        name: 'COVID19',
      },
    };
    await axios({
      ...
      headers: { Authorization: `Bearer ${process.env.SendgridToken}` },
      data: emailParams,
    })
      .then((res) => res.data)
      .catch((err) => err);
  ...
  if (phoneList.length > 0) {
    for (const phone of phoneList) {
      await axios({
        method: 'post',
        url: `https://api.twilio.com/2010-04-01/Accounts/${process.env.TwilioAccount}/Messages.json`,
        auth: {
          username: process.env.TwilioAccount,
          password: process.env.TwilioToken,
        },
                              ...
                              <tr>
                                ...
                                >
                                  <img
                                    ...
                                    "
                                    class="hund"
                                    src="https://www.eu-patient.eu/contentassets/f7fb4b5549994bd0857d44ac0382c018/cut_outs-1200x520.jpg"
                                  />
                                </td>
                              </tr>
                              <!-- END HEADER IMAGE -->
